...
def build_markdown(results: dict, winners: list[dict], results_prefix: str = "", title: bool = True) -> str:
    """results_prefix: relative path FROM the document being written TO the
    results/ directory. Empty for RESULTS.md (which lives inside results/
    itself); "results/" for README.md (which lives at the repo root)."""
    lines = (["# Results\n"] if title else []) + [
    ...
    lines.append("| Platform | Status | Client env | Timestamp (UTC) | Error |")
    lines.append("|---|---|---|---|---|")
    client_envs = set()
    for pid in PLATFORM_ORDER:
        r = results.get(pid)
        if r is None:
            ...
            env = r.get("client_env", "unspecified")
            if r["status"] == "ok":
                client_envs.add(env)
            lines.append(f"| {_platform_name(pid)} | {r['status']} | {env} | {r['timestamp_utc']} | {err} |")
    if len(client_envs) > 1:
        lines.append(
            f"\n> ⚠️ **These results were captured from {len(client_envs)} different client "
            f"environments ({', '.join(sorted(client_envs))})** — they are not a same-client, "
            "same-region comparison. Re-run every platform from the same `BENCHMARK_CLIENT_ENV` "
            "before treating latency numbers as comparable.\n"
        )
    lines.append("")
    ...
    for fname, caption in (
        ...
        lines.append(f"![{caption}]({results_prefix}charts/{fname})\n")

    lines.append(
        f"\nSee also: [`dashboard.html`]({results_prefix}dashboard.html) for a browsable summary, "
...
def inject_into_readme(section_md: str) -> None:
    """Replaces the content between README_START/README_END markers in
    README.md with section_md. The assignment requires the full results
    matrix live IN the README, not just linked from it — this keeps that
    section scriptably in sync with results/raw/*.json instead of relying on
    someone to hand-copy numbers after every run."""
    readme_path = config.ROOT / "README.md"
    text = readme_path.read_text()
    if README_START not in text or README_END not in text:
        print(f"WARNING: {README_START}/{README_END} markers not found in README.md — skipping injection.")
        return
    before, rest = text.split(README_START, 1)
    _, after = rest.split(README_END, 1)
    new_text = f"{before}{README_START}\n{section_md}\n{README_END}{after}"
...
def main() -> None:
    ...
    readme_section = build_markdown(results, winners, results_prefix="results/", title=False)
